...
6. When you enter any subproject repo (e.g. `www-adam`, `q-tools`), read that repo's local `AGENTS.md` before coding and follow its mandatory checklists. For new web-adam services, use `q webadam new-service <service_id> <Display Name> [path] [port] [order]`.
...
Триггеры: «забэкапься», «сделай бэкап себя», «обнови backup-репы», «синхронизируй бэкап».

Обязательное действие:
1. Прочитать: `/home/adam/.openclaw/workspace/skills/self-backup/SKILL.md`.
2. Выполнить синхронизацию обоих репозиториев (`~/.openclaw/workspace` и `~/.openclaw`) по инструкции навыка.
3. Не выполнять destructive git-операции без явного подтверждения владельца.

### Навык: «Телеграмм Исследование» / «ТГ исследование»

Триггеры: пользователь просит «сделай телеграмм исследование», «ТГ исследование», «исследуй канал Telegram», «скачай N сообщений канала и проанализируй».
...
### Навык: «TG Checklist» / нативные Telegram списки

Триггеры: «сделай список», «создай checklist», «отправь список кому-то», «добавь пункт в список», «убери пункт из списка», «отметь/сними метку».

Обязательное действие:
1. Прочитать: `/home/adam/.openclaw/workspace/skills/tg-checklist/SKILL.md`.
2. Для нативных списков использовать только `q tg` checklist-команды (не текст-имитацию чекбоксов).
3. После отправки/редактирования сообщать `message_id` и что именно изменено.

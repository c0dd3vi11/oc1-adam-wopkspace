...
## Эталон (по cron-задаче)

Ориентироваться на активный cron `daily backup repo hygiene (09:00 MSK)`:
- проверить изменения,
- проверить plaintext-секреты,
- commit где нужно,
- push: сначала `~/.openclaw/workspace`, потом `~/.openclaw`,
- никаких destructive-операций без явного подтверждения Антона.
